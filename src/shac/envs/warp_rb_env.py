class RigidBodySimulator:
    ...
    def init_sim(self):
        ...
        for i in range(self.num_envs):
            wp.sim.parse_urdf(
                os.path.join(os.path.dirname(__file__), self.urdf),
                self.builder,
                xform=wp.transform(
            ...
            builder.joint_target[: len(self.q_init)] = [
                0.0 for _ in range(len(self.q_init))
            ]
        ...
        if self.render:
            self.renderer = wp.sim.render.SimRenderer(
                self.model,
                os.path.join(os.path.dirname(__file__), f"outputs/{self.name}.usd"),
            )
    ...
    def warp_step(self, q, qd, tau, q_next, qd_next, requires_grad=False):
        if requires_grad:
            ...
            states = [
                self.model.state(requires_grad=False)
                for _ in range(self.sim_substeps + 1)
            ]

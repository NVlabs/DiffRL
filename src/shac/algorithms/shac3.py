...
class SHAC:
    ...
    def __init__(self, cfg: DictConfig):
        ...
        self.contact_truncation = cfg.alg.params.config.contact_truncation
        self.min_steps = 4
    ...
    def compute_actor_loss(self, deterministic=False):
        ...
        for i in range(self.steps_num):
            ...
            with torch.no_grad():
                _, mus_i, sigmas_i = self.actor.forward_with_dist(
            ...
            if self.critic_name == "q_network":
                ...
                val_model_free = self.target_critic(obs.detach(), next_actions).squeeze(
                    -1
                )
            elif self.critic_name == "double_q_network":
                val = torch.minimum(*self.target_critic(obs, next_actions)).squeeze(-1)
                val_model_free = torch.minimum(
                    *self.target_critic(obs.detach(), next_actions)
                ).squeeze(-1)
            elif self.critic_name == "value_network":
            ...
            for id in done_env_ids:
                if (
                    ...
                    if self.critic_name == "CriticMLP":
                        next_values[i + 1, id] = self.target_critic(real_obs).squeeze(
                            -1
                        )
                    else:
                        real_act = torch.tanh(self.actor(real_obs, deterministic=True))
                        next_values[i + 1, id] = self.target_critic(
                            real_obs, real_act
                        ).squeeze(-1)
        ...
        # self.actor_model_free_loss = actor_model_free_loss.detach().cpu().item()
    ...
    def train(self):
        ...
        for epoch in range(start_epoch, self.max_epochs):
            ...
            if len(self.episode_loss_his) > 0:
                ...
                self.writer.add_scalar(
                    "rollout_len/step", rollout_len_mean, self.step_count
                )
                self.writer.add_scalar(
                    "rollout_len/iter", rollout_len_mean, self.iter_count
                )
            else:
                rollout_len_mean = self.rollout_lens.mean().cpu().item()

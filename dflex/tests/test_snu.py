...
class HumanoidSNU:
    ...
    def __init__(self, depth=1, mode="numpy", render=True, adapter="cpu"):
        ...
        self.node_map, self.xform_map, self.mesh_map = test_util.parse_skeleton(
            "assets/snu/arm.xml", builder, self.filter
        )
        self.muscles = test_util.parse_muscles(
            "assets/snu/muscle284.xml", builder, self.node_map, self.xform_map
        )
    ...
    def loss(self):
        ...
        for i in range(0, self.sim_steps):
            # apply actions
            self.model.muscle_activation = (
                torch.tanh(4.0 * self.activations[0] - 2.0) * 0.5 + 0.5
                    ...
                    with torch.no_grad():
                        ...
                        for m in range(self.model.muscle_count):
                            ...
                            self.renderer.add_line_strip(
                                points,
                                name=self.muscles[m].name,
                                radius=0.0075,
                                color=(
                                    self.model.muscle_activation[m]
                                    / self.muscle_strength,
                                    0.2,
                                    0.5,
                                ),
        ...
        if self.name == "humanoid_snu_neck":
            ...
            target_dir = torch.tensor(
                (1.0, 0.0, 0.1), dtype=torch.float32, device=self.adapter
            )

            head_forward = transform_vector_torch(
                self.state.body_X_sc[self.node_map["Head"]], forward_dir
            )
            head_up = transform_vector_torch(
                self.state.body_X_sc[self.node_map["Head"]], up_dir
            )

            loss = (
                loss
                - torch.dot(head_forward, target_dir) * self.target_penalty
                - torch.dot(head_up, up_dir) * self.target_penalty
            )

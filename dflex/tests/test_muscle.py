...
class MuscleTest:
    ...
    def __init__(self, depth=1, mode="numpy", render=True, adapter="cpu"):
        ...
        for i in range(2):
            if i == 0:
                body = builder.add_link(
                    body,
                    df.transform((2.0 * length, 0.0, 0.0), df.quat_identity()),
                    axis=(0.0, 0.0, 1.0),
                    damping=1.0,
                    stiffness=0.0,
                    type=df.JOINT_FIXED,
            ...
            shape = builder.add_shape_box(
                body, pos=(length, 0.0, 0.0), hx=length, hy=width, hz=width
            )

        builder.add_muscle(
            [0, 1],
            [(length * 2.0 - 0.25, width + 0.05, 0.0), (0.25, width + 0.05, 0.0)],
            1.0,
            1.0,
            1.0,
            1.0,
        ...
        self.activations = torch.zeros(
            (self.sim_steps, 2), dtype=torch.float32, device=adapter, requires_grad=True
        )
                    ...
                    with torch.no_grad():
                        for m in range(self.model.muscle_count):
                            ...
                            self.renderer.add_line_strip(
                                ...
                                name=("muscle_0" + str(m)),
                                radius=0.02,
                                color=(self.activations[i][m] / 1000.0 + 0.5, 0.2, 0.5),

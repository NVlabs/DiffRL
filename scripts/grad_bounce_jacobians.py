...
class Bounce:
    ...
    def __init__(
        ...
        self.model.soft_contact_kd = 1e2  # damping
...
def main(config: DictConfig):
    ...
    for h in tqdm(range(H, H + 1)):
        if h == H:
            env = Bounce(ww, num_envs=N, num_steps=h, profile=False, render=True)
        else:
    ...
    for i in tqdm(range(env.sim_steps)):
        ...
        with tape:
            ...
            jacobian = np.empty((N, output_dim, input_dim), dtype=np.float32)

            for out_idx in range(output_dim):
                input = env.states[i].particle_qd
                output = env.states[i + 1].particle_qd
                ...
                tape.backward(grads={output: e})
                q_grad_i = tape.gradients[input]
                jacobian[:, out_idx, :] = q_grad_i.numpy().reshape(N, input_dim)
                tape.zero()

            jacs.append(jacobian)

    jacs = np.array(jacs)
    print("jacs", jacs.shape)
    np.save("jacs", jacs)
